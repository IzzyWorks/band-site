...
function getComments() {
    axios
      ...
      .then(function (response) {
        console.log(response);  // 1st console.log the response
        appendCommentBlock(response.data); // pass data to another function to work with
      })
      .catch(function (error) {
        console.log(error); // handle errors that may occur when communicating with API/server
      });
  }
...
// getComment.then(result) => {console.log(listPostedComments);.catch(err =>{listPostedComments.log(err);})
...
function appendCommentBlock() {
    ...
    for (let i = 0; i < listPostedComments.length; i++) {
      const commentObj = response.data[i];   //this is currently where js gets the array data
